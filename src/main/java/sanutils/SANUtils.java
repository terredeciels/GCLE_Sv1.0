...
public class SANUtils {
    ...
    public static GCoups toMove(final GPositionS gPosition, final String pSAN) throws SANException {
        ...
        if ("0-0".equals(pSAN)) {
            ...
            if (trait) {
                return new GCoups(ICodage.ROI(), ICodage.e1(), ICodage.g1(), ICodage.h1(), ICodage.f1(), 0, TypeDeCoups.Roque(), 0);

            }
            return new GCoups(ICodage.ROI(), ICodage.e1(), ICodage.c1(), ICodage.a1(), ICodage.d1(), 0, TypeDeCoups.Roque(), 0);
        } else if ("0-0-0".equals(pSAN)) {
            // ... les grands roques...
            if (trait) {
                return new GCoups(ICodage.ROI(), ICodage.e1(), ICodage.c1(), ICodage.a1(), ICodage.d1(), 0, TypeDeCoups.Roque(), 0);
            }
            return new GCoups(ICodage.ROI(), ICodage.e8(), ICodage.g8(), ICodage.h8(), ICodage.f8(), 0, TypeDeCoups.Roque(), 0);
        }
        ...
        if (Character.isLowerCase(c)) {
            if (trait) {
                piece = ICodage.BLANC() * ICodage.PION();
            } else {
                piece = ICodage.NOIR() * ICodage.PION();
            }
        } else {
            if (trait) {
                piece = valueOf(c);
...
//        System.out.println("mvts83= " + mvts);
        ...
        if ((mvts.size() > 1) && (((piece == ICodage.NOIR() * ICodage.PION()) && (getRank(dst) == 0))
                || ((piece == ICodage.BLANC() * ICodage.PION()) && (getRank(dst) == 7)))) {
    ...
    private static int valueOf(final int pColonne, final int pLigne) {
        ...
        return ICodage.CASES117()[pColonne + pLigne * 8];
    }

    private static int valueOf(char c) {
        int color = Character.isLowerCase(c) ? ICodage.NOIR() : ICodage.BLANC();
        char t = Character.toLowerCase(c);
        switch (t) {
            case 'k':
                return color * ICodage.ROI();
            case 'q':
                return color * ICodage.DAME();
            case 'b':
                return color * ICodage.FOU();
            case 'n':
                return color * ICodage.CAVALIER();
            case 'r':
                return color * ICodage.TOUR();
            default:
                return 0; //erreur
        }
    }
    ...
    public static String toSAN(GPositionS gPosition, final GCoups pMouvement) {
        ...
        if ((t == ICodage.ROI()) && (Math.abs(xSrc - xDst) > 1)) {
            // Roques...
            sb.append("0-0");
            if (xSrc > xDst) {
                sb.append("-0");
...
//                System.out.println(mvts);
            ...
            if (t == ICodage.PION()) {
                ...
                if (dst == gPosition.getCaseEP()) {
                    ...
                    if ((trait && (yDst == 8 - 1))
                        ...
                        if (promotion == 0) {
                            assert false;
                            sb.append(getSANLetter(ICodage.DAME()));
                        } else {
                            final int promotionType = Math.abs(promotion);
                            if (promotionType != ICodage.PION()) {
    ...
    private static String getFENString(int dst) {
        return String.valueOf((char) ('a' + getFile(dst))) + (char) ('1' + getRank(dst));
    }

    public static int getFile(int _case) {
        return (_case - 12 * (_case / 12)) - 2;
    }
